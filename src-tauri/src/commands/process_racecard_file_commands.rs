...
pub async fn process_racecard_file<'a>(path: String) -> Result<Racecard, String> {
    ...
    for line in &lines {
        ...
        let mut horse = Horse {
            ...
            best_bris_speed_fast_track: line[SF_BRIS_BEST_SPEED_FAST_TRACK].parse::<u32>().ok(),
            best_bris_speed_turf: line[SF_BRIS_BEST_SPEED_TURF].parse::<u32>().ok(),
            best_bris_speed_off_track: line[SF_BRIS_BEST_SPEED_OFF_TRACK].parse::<u32>().ok(),
            best_bris_speed_distance: line[SF_BRIS_BEST_SPEED_DISTANCE].parse::<i32>().ok(),
        ...
        for j in 0..10 {
            let pp = PastPerformance { 
                ...
                bar_shoes: line[SF_PP_BAR_SHOE + j].clone(),
