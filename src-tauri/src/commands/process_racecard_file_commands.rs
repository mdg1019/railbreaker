...
pub async fn process_racecard_file<'a>(path: String) -> Result<Racecard, String> {
    ...
    for line in &lines {
        ...
        let mut horse = Horse {
            ...
            lifetime_all_weather_earnings: line[SF_LIFETIME_ALL_WEATHER_EARNINGS].parse::<u32>().ok(),
            best_bris_speed_all_weather_surface: line[SF_BEST_BRIS_SPEED_ALL_WEATHER].parse::<u32>().ok(),
            ...
            best_bris_speed_most_recent_year: line[SF_BEST_BRIS_SPEED_MOST_RECENT_YEAR].parse::<u32>().ok(),
            best_bris_speed_2nd_most_recent_year: line[SF_BEST_BRIS_SPEED_2ND_MOST_RECENT_YEAR].parse::<u32>().ok(),
            ...
            jockey_distance_turf_earnings: line[SF_JOCKEY_DISTANCE_TURF_EARNINGS].parse::<u32>().ok(),
            post_times: line[SF_POST_TIMES].clone(),
            trainer_jockey_combo_starts_meet: line[SF_TRAINER_JOCKEY_COMBO_STARTS_MEET].parse::<u32>().ok(),
            trainer_jockey_combo_wins_meet: line[SF_TRAINER_JOCKEY_COMBO_WINS_MEET].parse::<u32>().ok(),
            trainer_jockey_combo_places_meet: line[SF_TRAINER_JOCKEY_COMBO_PLACES_MEET].parse::<u32>().ok(),
            trainer_jockey_combo_shows_meet: line[SF_TRAINER_JOCKEY_COMBO_SHOWS_MEET].parse::<u32>().ok(),
            trainer_jockey_combo_roi_meet: line[SF_TRAINER_JOCKEY_COMBO_ROI_MEET].parse::<f64>().ok(),
        ...
        for j in 0..12 {
            let workout = Workout {
                ...
                workouts_that_day_distance: line[SF_WORKOUT_WORKOUTS_THAT_DAY_DISTANCE + j].parse::<u32>().ok(),
        ...
        for j in 0..10 {
            let pp = PastPerformance { 
                ...
                start_call_between_lengths_leader: line[SF_PP_START_CALL_BETWEEN_LENGTHS_LEADER_MARGIN + j].parse::<f64>().ok(),
                start_call_between_lengths: line[SF_PP_START_CALL_BETWEEN_LENGTHS + j].parse::<f64>().ok(),
                first_call_between_lengths_leader: line[SF_PP_1ST_CALL_BETWEEN_LENGTHS_LEADER_MARGIN + j].parse::<f64>().ok(),
                first_call_between_lengths: line[SF_PP_1ST_CALL_BETWEEN_LENGTHS + j].parse::<f64>().ok(),
                second_call_between_lengths_leader: line[SF_PP_2ND_CALL_BETWEEN_LENGTHS_LEADER_MARGIN + j].parse::<f64>().ok(),
                second_call_between_lengths: line[SF_PP_2ND_CALL_BETWEEN_LENGTHS + j].parse::<f64>().ok(),
                bris_race_shape_1st_call: line[SF_PP_BRIS_RACE_SHAPE_1ST_CALL + j].parse::<u32>().ok(),
                stretch_call_between_lengths_leader: line[SF_PP_STRETCH_BETWEEN_LENGTHS_LEADER_MARGIN + j].parse::<f64>().ok(),
                stretch_call_between_lengths: line[SF_PP_STRETCH_BETWEEN_LENGTHS + j].parse::<f64>().ok(),
                finish_between_lengths_leader: line[SF_PP_FINISH_BETWEEN_LENGTHS_LEADER_MARGIN + j].parse::<f64>().ok(),
                finish_between_lengths: line[SF_PP_FINISH_BETWEEN_LENGTHS + j].parse::<f64>().ok(),
                bris_race_shape_2nd_call: line[SF_PP_BRIS_RACE_SHAPE_2ND_CALL + j].parse::<u32>().ok(),  
                ...
                apprentice_weight_allowance: line[SF_PP_APPRENTICE_WEIGHT_ALLOWANCE + j].parse::<u32>().ok(),
                ...
                bris_speed_par_for_race: line[SF_PP_BRIS_SPEED_PAR_FOR_RACE + j].parse::<u32>().ok(),
                bar_shoes: line[SF_PP_BAR_SHOES + j].clone(),
                company_line_codes: line[SF_PP_COMPANY_LINE_CODES + j].clone(),
                low_claiming_price_of_race: line[SF_PP_LOW_CLAIMING_PRICE_OF_RACE + j].parse::<u32>().ok(),
                high_claiming_price_of_race: line[SF_PP_HIGH_CLAIMING_PRICE_OF_RACE + j].parse::<u32>().ok(),
                code_for_prior_races: line[SF_PP_CODE_FOR_PRIOR_RACES + j].clone(),
                claimed_and_trainer_switches_1: line[SF_PP_CLAIMED_AND_TRAINER_SWITCHES_1 + j].clone(),
                claimed_and_trainer_switches_2: line[SF_PP_CLAIMED_AND_TRAINER_SWITCHES_2 + j].clone(),
                claimed_and_trainer_switches_3: line[SF_PP_CLAIMED_AND_TRAINER_SWITCHES_3 + j].clone(),
                claimed_and_trainer_switches_4: line[SF_PP_CLAIMED_AND_TRAINER_SWITCHES_4 + j].clone(),
                claimed_and_trainer_switches_5: line[SF_PP_CLAIMED_AND_TRAINER_SWITCHES_5 + j].clone(),
                claimed_and_trainer_switches_6: line[SF_PP_CLAIMED_AND_TRAINER_SWITCHES_6 + j].clone(),
                extended_start_comment: line[SF_PP_EXTENDED_START_COMMENT + j].clone(),
                sealed_track_indicator: line[SF_PP_SEALED_TRACK + j].clone(),
                previous_all_weather_surface_indicator: line[SF_PP_PREVIOUS_ALL_WEATHER_SURFACE_INDICATOR + j].clone(),
                equibase_abbreviated_race_condition: line[SF_PP_EQUIBASE_ABBREVIATED_RACE_CONDITIONS + j].clone(),
            };
...
fn prepend_weekday(s: &str) -> Option<String> {
    ...
    if normalized.contains('/') {
        ...
        if parts.len() == 3 {
            ...
            if m < 3 { y -= 1; }
            let w = (y + y/4 - y/100 + y/400 + t[(m - 1) as usize] + d) % 7; // 0 = Sunday
    ...
    let day = day_str.chars().filter(|c| c.is_ascii_digit()).collect::<String>().parse::<i32>().ok()?;
